<?php 
	...
	function as_navigation($request, $html = '')
	{
		...
		$navigation = array();
		if ( $adminid ) {			
			$navigation['bookings'] = array('label' => 'Bookings', 'url' => 'admin.php?open=booking_all');
			$navigation['clients'] = array('label' => 'Clients', 'url' => 'admin.php?open=client_all');
			$navigation['rooms'] = array('label' => 'Rooms', 'url' => 'admin.php?open=room_all');
			$navigation['admins'] = array('label' => 'Admins', 'url' => 'admin.php?open=admin_all');
			$navigation['settings'] = array('label' => 'Settings', 'url' => 'admin.php?open=settings');			
			$navigation['signout'] = array('label' => 'Sign Out?', 'url' => 'admin.php?open=signout');
		} else {
			$navigation['signin'] = array('label' => 'Sign In', 'url' => 'admin.php?open=signin');
			$navigation['register'] = array('label' => 'SignUp', 'url' => 'admin.php?open=register');
		}			
	...
	<h2>
			...
			case 'table': ?>
				...
				<th><?php echo $header ?></th>
					...
					<?php foreach($page['items'] as $rid => $trow) { ?>
						<tr onclick="location='admin.php?<?php echo $page['onclick'].$rid ?>'">
		...
		<?php break;
			...
			<?php foreach($page['items'] as $item) { ?>
				<div title="<?php echo $item[6] ?>"><a href="admin.php?open=job_view&&jobid=<?php echo $item[0] ?>"><h3><?php echo $item[1] ?></h3></a>
				<p>
