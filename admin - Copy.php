<?php
	...
	switch ( $open ) {
		...
		case 'room_view':
			...
			$content['link'] = '<a href="admin.php?open=room_delete&&roomid='.$roomid.'" onclick="return confirm(\'Delete This Room? This action is irrevesible!\')" style="float:right;">DELETE ROOM</a>';	
			$content['page'] = array(
					'type' => 'form',
			...
			if ( isset( $_POST['saveChanges'] ) ) {
				$class->storeFormValues( $_POST );
				$class->update();
				header( "Location: admin.php?open=room_view&&roomid=".$roomid."status=changesSaved" );
		...
		case 'room_all':
			...
			$content['link'] = '<a href="admin.php?open=room_new" style="float:right">Add a Room</a>';
